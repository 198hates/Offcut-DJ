    ...
    <div className={`flex-1 min-w-[200px] rounded-lg border ${status.isOnAir ? 'border-green-500/40 bg-green-500/[0.04]' : 'border-border/30 bg-ink/[0.03]'} p-3 space-y-2.5`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-muted">CDJ {status.deviceId}</span>
          {status.isMaster && (
            <span className="px-1 py-0.5 rounded bg-accent/20 font-mono text-[8px] text-accent uppercase tracking-[0.1em]">master</span>
          )}
          {status.isSync && (
            <span className="px-1 py-0.5 rounded bg-border/30 font-mono text-[8px] text-muted uppercase tracking-[0.1em]">sync</span>
          )}
        </div>
        <div className="flex items-center gap-1.5">
          <div className={`w-2 h-2 rounded-full ${stateDot} transition-all`} />
          <span className={`font-mono text-[9px] uppercase tracking-[0.12em] ${stateColor}`}>
            {status.playState}
          </span>
        </div>
      </div>
      ...
      {/* Track info */}
      {isEmpty ? (
        <p className="font-mono text-[10px] text-muted/50 italic">no track loaded</p>
      ) : (
        <div className="space-y-0.5">
          <p className="font-mono text-[11px] text-ink leading-tight truncate" title={status.title ?? undefined}>
            {status.title ?? <span className="text-muted italic">unknown track</span>}
          </p>
          {status.artist && (
            <p className="font-mono text-[9px] text-muted truncate">{status.artist}</p>
          )}
          {status.label && (
            <p className="font-mono text-[8px] text-muted/60 truncate uppercase tracking-[0.08em]">{status.label}</p>
          )}
        </div>
      )}
      ...
      {!isEmpty && (
        <div className="flex items-center gap-3 pt-0.5">
          <div className="space-y-0.5">
            <p className="font-mono text-[8px] uppercase tracking-[0.12em] text-muted">bpm</p>
            <p className="font-mono text-[11px] text-ink">{fmtBpm(status.trackBPM)}</p>
          </div>
          {status.key && (
            <div className="space-y-0.5">
              <p className="font-mono text-[8px] uppercase tracking-[0.12em] text-muted">key</p>
              <p className="font-mono text-[11px] text-ink">{status.key}</p>
            </div>
          )}
      ...
      {status.isOnAir && (
        <div className="flex items-center gap-1 pt-0.5">
          ...
          <span className="font-mono text-[8px] uppercase tracking-[0.14em] text-green-400/80">on air</span>
        </div>
      )}
    </div>
  )
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-border/30 flex items-center justify-between shrink-0 bg-chassis">
        <div className="flex items-center gap-2.5">
          ...
          <h1 className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-ink">ProLink Capture</h1>
          {isActive && (
            <span className="px-1.5 py-0.5 rounded bg-green-500/15 border border-green-500/30 font-mono text-[8px] text-green-400 uppercase tracking-[0.12em] animate-pulse">live</span>
          )}
          {isConnecting && (
            <span className="px-1.5 py-0.5 rounded bg-accent/15 border border-accent/30 font-mono text-[8px] text-accent uppercase tracking-[0.12em]">connecting…</span>
          )}
          {isStopping && (
            <span className="px-1.5 py-0.5 rounded bg-border/30 font-mono text-[8px] text-muted uppercase tracking-[0.12em]">stopping…</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {isActive ? (
            <button
              onClick={handleStop}
              disabled={isStopping}
              className="px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.1em] bg-red-500/80 hover:bg-red-500 text-paper rounded transition-colors disabled:opacity-50"
            >
              stop capture
            </button>
          ) : isIdle ? (
            <button
              ...
              className="px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.1em] bg-accent hover:bg-accent/90 text-paper rounded transition-colors disabled:opacity-50"
            >
              start capture
            </button>
          ) : null}
        ...
        {errorMsg && (
          <div className="px-5 py-2.5 bg-red-500/10 border-b border-red-500/20 flex items-start gap-2 shrink-0">
            ...
            <div className="flex-1">
              ...
              <p className="font-mono text-[9px] text-muted/70 mt-0.5">
        ...
        {isIdle && !isConnecting && (
          <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6">
            ...
            <div className="space-y-3 max-w-md">
              <div>
                <h2 className="font-mono text-[9px] uppercase tracking-[0.15em] text-muted mb-1">network interface</h2>
              ...
              {ifaces.length === 0 ? (
                <p className="font-mono text-[10px] text-muted italic">No IPv4 interfaces found</p>
              ) : (
                <select
                  value={selectedIface}
                  onChange={(e) => setSelectedIface(e.target.value)}
                  className="bg-paper border border-border/40 rounded px-2 py-1.5 font-mono text-[10px] text-ink outline-none focus:border-accent w-full max-w-sm"
                >
            ...
            <div className="space-y-2 max-w-md">
              <h2 className="font-mono text-[9px] uppercase tracking-[0.15em] text-muted">pre-flight</h2>
              {[
                ...
                <div key={check} className="flex items-start gap-2">
                  ...
                  <span className="font-mono text-[9px] text-muted/70">{check}</span>
                </div>
              ))}
            </div>
            ...
            <div className="max-w-md bg-ink/[0.03] border border-border/20 rounded-lg px-3 py-2.5 space-y-1">
              <p className="font-mono text-[8px] uppercase tracking-[0.12em] text-muted/60">note</p>
              <p className="font-mono text-[9px] text-muted/70 leading-relaxed">
        ...
        {(isActive || isStopping) && (
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="px-4 py-3 border-b border-border/20 shrink-0">
              <p className="font-mono text-[8px] uppercase tracking-[0.14em] text-muted/60 mb-2">
              ...
              {playerStatuses.length === 0 ? (
                <div className="flex items-center gap-2 text-muted">
                  ...
                  <span className="font-mono text-[10px]">waiting for CDJs…</span>
                </div>
              ) : (
              ...
              {capturedTracks.length === 0 ? (
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center space-y-1">
                    <p className="font-mono text-[10px] text-muted/40">no tracks captured yet</p>
                    <p className="font-mono text-[9px] text-muted/30">
